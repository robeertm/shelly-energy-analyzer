...
class CoreMixin:
    ...
    def _build_sync_tab(self) -> None:
            ...
            # Network traffic section – use grid so Treeview doesn't dictate width
            traffic_lf = ttk.LabelFrame(frm, text=self.t("traffic.title"))
            traffic_lf.pack(fill="x", padx=12, pady=(0, 10))
            traffic_lf.columnconfigure(0, weight=1)
            # Rate + totals row
            rate_frm = ttk.Frame(traffic_lf)
            rate_frm.grid(row=0, column=0, sticky="ew", padx=8, pady=(6, 2))
            ...
            # Category breakdown (Treeview table) with scrollbar – in own frame with clipping
            tree_frm = ttk.Frame(traffic_lf)
            tree_frm.grid(row=1, column=0, sticky="ew", padx=8, pady=(2, 2))
            tree_frm.columnconfigure(0, weight=1)
            tree_frm.grid_propagate(False)
            tree_frm.configure(height=180)
