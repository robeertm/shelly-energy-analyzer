...
class EnergyDB:
    ...
    def _backfill_n_current(self, conn: sqlite3.Connection) -> None:
        ...
        try:
            ...
            # Use COALESCE to prefer avg columns, fall back to base columns
            ia = "COALESCE(a_avg_current, a_current, 0)"
            ib = "COALESCE(b_avg_current, b_current, 0)"
            ic = "COALESCE(c_avg_current, c_current, 0)"
            # SQLite doesn't have SQRT, so we use a power trick: x^0.5
            # Actually, SQLite doesn't have POWER either. Use a subquery approach.
            # We'll batch-update in Python instead.
            cursor = conn.execute(
