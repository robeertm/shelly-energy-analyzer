...
def _on_metric_tab_changed(_evt: Any = None) -> None:
    ...
    try:
        ...
        midx = 0
    metric_key = self._plots_metric_key_order[midx] if 0 <= midx < len(self._plots_metric_key_order) else "kwh"
    dev_nb = self._plots_device_nb.get(metric_key)
    if dev_nb is None:
        return
    try:
        target = int(getattr(self, "_plots_last_device_idx", 0))
    except Exception:
        target = 0
    try:
        tabs = list(dev_nb.tabs())
        if tabs:
            if target < 0:
                target = 0
            if target >= len(tabs):
