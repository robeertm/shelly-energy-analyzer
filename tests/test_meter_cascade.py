...
def test_removing_last_reading_resets_scalar():
    """a user's follow-up: a derived grid factor exists (scalar mirrors it), then the
...
def test_reconcile_heals_persisted_stale_child_scalar():
    """a user's live case: the −67 % stayed even after the reset-on-delete fix,
    ...
    print("OK  loading a config heals a stale main-meter-child scalar (a user's −67 %)")
