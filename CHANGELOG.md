...
- **Gen1 Shelly EM / 3EM (SHEM-3) no longer shows "Offline / no data" while it is
  ...
  poll failed, so no live sample was produced and the device stayed permanently
  offline in the Devices panel and Health Check — even though `/status` returned
  live power readings and historical CSV import worked. `get_em_status` is now
  generation-aware: for `gen == 1` it reads `/status` and normalizes the
...
- **Embodied CO₂ and solar footprint (CO₂ tab).** Grid-only accounting counted self-consumed solar and battery output as zero carbon. They are not: manufacturing the panels and cells emits CO₂ that is amortised over the energy they deliver. The CO₂ tab now adds a **Solar footprint (this month)** card — net footprint and effective intensity (g/kWh), the operational grid share, the **embodied** CO₂ of self-consumed PV and of battery-delivered energy, and the CO₂ **avoided by solar** versus buying the same energy from the grid. Two new lifecycle factors are configurable under Solar: `pv_embodied_g_per_kwh` (default 40 g/kWh, IPCC median for rooftop PV) and `battery_embodied_g_per_kwh` (default 60 g/kWh discharged).
### Fixed
- **External battery / PV / grid devices showed 0.000 kWh and contributed no energy to any calculation.** The external PV source ingested only instantaneous power for its synthetic `pv`, `battery` and `grid_ext` devices; the hourly energy rollup sums per-sample interval energy, which was never written, so every rollup was zero (visible as a battery stuck at 0.000 kWh, and PV/battery contributing nothing to autarky, sankey, costs or CO₂). The source now integrates interval energy between samples (trapezoidal, capped at 10 min like the Shelly ingest) and writes it with each sample, and maintains a per-device daily energy accumulator so the Live tile shows a meaningful "today" figure (PV produced, battery discharged, grid imported).
### Changed
- **The Home Assistant add-on now persists all Solar settings.** The add-on regenerates `config.json` from its options on every start, so Solar fields absent from the options schema were reset on restart. `kw_peak`, `battery_kwh`, `co2_production_kg_per_kwp` and the two new embodied-CO₂ factors are now part of the add-on options/schema and survive restarts.
...
- **Meter cascade (hierarchy).** Calibration used to be strictly per device. Anyone with several sub-meters behind ONE main meter (e.g. "House" + "Wallbox") ended up comparing the main-meter reading against a SINGLE sub-device — which yields a nonsensical factor (the main meter measures both together). You can now model an arbitrary meter structure: any number of **main meters** (with name + serial number), and every Shelly device is assigned to its parent meter (a main meter or another measuring device → arbitrary cascade depth). A main meter always calibrates against the **sum of its direct `em` children** — switches and deeper sub-sub-meters are never counted (no double counting).
  - Backend: new `MainMeter` (id/name/serial) + `AppConfig.main_meters`, `DeviceConfig.parent`; endpoints `GET/POST /api/meters`, `DELETE /api/meters/<id>` (detaches assigned devices instead of orphaning them). Fully **backward compatible**: existing (flat) configs load unchanged (`parent=""`, `main_meters=[]`).
- **Meter-reading log instead of start/end entry.** For each main meter you log the **meter reading** from time to time (`MeterReading{ts,kwh}`); the app derives the calibration factor automatically from the deviation BETWEEN consecutive readings and applies it **dated and retroactively** to exactly that period. A single reading is the starting value (no factor yet). Older readings can be inserted at any time (the affected period is recomputed). **Trend chart** of the deviation over time, readings table with "factor from here" + delete. Endpoints `POST/DELETE /api/meters/<id>/reading`. Drift-free via raw (uncompensated) kWh in the back-calculation (`query_hourly(compensate=False)`).
- **Factor for consumption BEFORE the first reading.** That pre-period has no measurement of its own and is extrapolated with the **consumption-weighted overall factor** (total meter Δ / total raw Δ across all intervals) — not the simple mean of the percentages, which over-weights short/noisy intervals. Implemented as a dated pre-period step so the existing step function applies it retroactively.
- **Setup wizard: "Calibrated main meter" step.** Right after the Devices step you can create the official (calibrated) grid meter with its **serial number**, assign the matching Shelly sub-meters, and enter the **current meter reading as the starting value** — so there is a calibrated baseline from day one. Skippable; first-time setup only.
- New UI strings (`cal.*`, `setup.mainmeter.*`) in DE + EN; full 9-language sweep to follow.
...
- **`GET /api/v1/<anything>` always returned `handle_v1_request() missing 2 required positional arguments: 'db' and 'cfg'`.** The Flask dispatcher in `web/blueprints/api_data.py` was calling the v1 router with only `(path, state)` — the router itself expects `(path, params, db, cfg)`. Every `/api/v1/...` call therefore raised a `TypeError` that the route's broad `except Exception` swallowed into the user-facing JSON error, so the entire v1 API surface (`/devices`, `/costs`, `/spot_prices`, `/co2`, `/status`, `/openapi.json`) was dead. Dispatcher now passes `dict(request.args), state.storage, state.cfg`. Added a `/health` (alias `/healthz`) endpoint returning `{"ok": true, "data": {"status": "ok"}}` so external monitors / Home Assistant add-on healthchecks have a fast, side-effect-free probe.
...
- **Time-stamped calibration history (per device) + new Calibration tab.** Measurement compensation is no longer a single scalar that retroactively rewrites the entire history — every device now carries a `compensation_history` list of time-stamped entries (`effective_from_ts`, `percent`, `note`, plus the meter reading the entry was derived from when available). The DB read layer applies the right factor for each sample row using a vectorized step-function lookup (`numpy.searchsorted` against the entry boundaries), so reading any historical range automatically picks up the calibration that was in force at that moment. Samples whose timestamp is older than the first history entry fall back to the legacy `compensation_percent` scalar, so existing configs keep working unchanged.
  - **API:** `GET /api/compensation/history[?device=K]`, `POST /api/compensation/history` (direct percent OR derived from a meter-reading delta over a chosen time range), `DELETE /api/compensation/history` (by `effective_from_ts`).
  - **UI:** new top-level *Calibration* tab (📏) lists each device with (1) a step-function line chart of compensation % over time (one dot per entry, line stretches to *now*, hover-tooltip with date + percent + note), (2) a sortable table with Δ to the previous entry and the meter / raw kWh that produced it, and (3) an "Add entry" form that toggles between **Direct %** and **Compute from meter reading** modes.
  - **Live data:** the realtime / MQTT pipeline now picks up the latest history entry as the *current* factor (no scalar mirror loss-of-precision), so a freshly-added calibration takes effect on the next sample without restarting the service.
...
- **EV charging log — dramatically faster and prettier.**
  Backend: `detect_charging_sessions` was rewritten as a vectorized numpy routine instead of iterating each sample in a Python `for` loop with `iterrows()`. On a typical 30-day wallbox history (~86k samples) detection now runs in ~2 ms instead of the multi-second loop that previously made the `/api/ev_sessions` request feel hung. An in-process detection cache short-circuits repeat calls until new samples arrive (sub-millisecond on cache hit). The endpoint also limits the underlying DB read to the requested time window (default last 90 days, `?days=N` overrides, 1..730), so /api/ev_sessions no longer drags in unbounded history.
  Frontend: the EV-Log tab is no longer a dense grey table. New layout has (1) a time-window filter bar (7d / 30d / 90d / 1y) wired to the new `?days=` backend parameter, (2) a top overview card with sessions / total energy / total cost / ⌀ per session / ⌀ duration, (3) a daily-energy heatmap of the last 30 days (one cell per day, intensity = kWh delivered, hover for the exact value), and (4) sessions rendered as readable cards — date, duration, peak & average power, kWh and cost on the right, with a per-session fill bar and a small trash icon to remove an entry. All EV-log UI strings now go through the i18n layer; the delete confirmation is no longer a hard-coded German `confirm()`.
...
- Removed from git tracking (kept locally where the tool needs them): `src/shelly_energy_analyzer.egg-info/` (auto-regenerated by `pip install -e .`), `.claude/` (settings + a leftover `worktrees/elastic-zhukovsky` ghost path from an earlier session), `.vscode/settings.json`.
- Deleted entirely (vestigial debug dumps from the desktop-app era, never referenced by current code): `_test.js`, `_control_test.js`, `live_dashboard.html` (~263 KB). No runtime impact, no API changes.
...
## 16.26.5 - 2026-04-19
### Fixed
- **iOS Scriptable widget no longer silently fails when the widget parameter is an IP address.** Tailscale / Let's Encrypt certs only include a DNS SAN, not raw IPs, and Scriptable enforces strict TLS — so a user who entered `10.0.0.42:8765` or the `100.x.y.z` Tailscale IP would hit "no alternative certificate subject name matches target" and see a generic "Offline" box with no hint of what went wrong.
...
- **Enhanced daily + monthly reports (Telegram / email / webhook).** User feedback: the scheduled reports sent via Telegram and email looked sparse — total kWh, a device list, and a single 2-panel chart. This release triples the content and adds a fully styled HTML email layout.
  - **Rich data pipeline.** `_build_daily_data()` and `_build_monthly_data()` in `web/background.py` do one query pass over SQLite and return a stat dict that feeds text, HTML and chart builders in lock-step — no duplicated queries, no drift between outputs. The dicts carry: total_kwh / total_cost, vs. previous day + vs. same weekday last week, per-device (kwh, cost, share_pct, delta_pct, peak W, peak hour), hourly totals, hourly per-device, max power + load factor, avg W, peak hour + top/bottom 3 hours, night base load + projected standby cost, CO₂, month-to-date projection, dynamic-spot cost comparison (via `query_spot_prices`), and the "biggest mover" device (largest absolute kWh swing vs prior period). The monthly variant adds daily totals per day, weekday averages (Mon–Sun with a weekday vs weekend ratio), hour-of-day aggregate across the whole month, best/worst day, CO₂ total + tree-days, and year projection.
  - **Expanded text bodies** (`_format_daily_text`, `_format_monthly_text`) — shown in Telegram and as the plain-text email fallback. Headline KPI block with arrows, device breakdown with share %, biggest mover line, top-3 busiest + lowest 3 hours, max power + load factor + avg W, night base load with annual kWh/€ projection, CO₂ footprint, dynamic spot cost vs. fixed, month projection, and an 8-bucket 24h text bar chart. Monthly adds weekday average line, weekend vs. weekday ratio, busiest hour of day, and a full daily profile text bar chart.
  - **4-panel PNG chart.** The chart PNG went from 2 to 4 subplots (1100×700 px, dark theme matching the dashboard). Daily: stacked hourly per device, device totals bar with kWh + € labels on each bar, cumulative cost curve with peak-hour marker, total hourly profile with night base-load reference line colour-coded red/amber/blue by load level. Monthly: daily bars vs average line, device donut chart with share %, weekday average (weekdays blue + weekend orange), hour-of-day aggregate across the whole month. Both have a centred title banner ("Shelly Energy Analyzer · Daily Report · ..."), consistent palette, and bigger typography.
  - **HTML email body with inline CSS** (`_format_daily_html`, `_format_monthly_html`). Gradient header (blue→purple), 4 colour-coded KPI cards (Total energy · Total cost · Max power · CO₂ — monthly uses Daily average instead of Max power), embedded chart image via `cid:` reference (no external fetches needed for preview in any mail client), device table with coloured dots + share + delta arrows (▲/▼/→) + peak hour, extras strip (night base load, top-3 hours, biggest mover, spot cost comparison, month projection), generated-at footer. Dark-themed (#0b0f14 / #121821) so it matches the dashboard look. Loads correctly in Apple Mail, Gmail web, Outlook desktop, Thunderbird, and mobile Gmail — uses table-based layout and inline styles as required by email clients.
  - **`_email_send()` gained `html_body` + `inline_images` params.** The SMTP payload is now a `multipart/mixed` → `multipart/related` → `multipart/alternative` tree: clients that prefer HTML get the rich layout, clients that only speak plain text fall back to the existing message, and the chart travels as an inline `MIMEImage` with a `Content-ID` header so `<img src="cid:daily-chart.png">` resolves without the preview pane downloading anything external.
  - **Summary loop re-wired** to call the new builders once per report and share the data dict across text, chart, HTML and PDF outputs. Email subjects dropped the German "Tagesbericht"/"Monatsbericht" labels and are now "Shelly Energy – Daily Report" / "Shelly Energy – Monthly Report".

### Fixed
- **Every remaining hard-coded German string in `web/templates/settings.html`** (171 replacements in one pass). The settings page schema arrays still had German labels, hints and select options across every section — Display, Web server, Pricing & tariff, TOU, Auto-sync, Telegram, Email, Webhook, Anomalies, MQTT, InfluxDB, Prometheus, API, Solar, Battery, PV surplus, Spot prices (details), CO₂, Weather, Forecast, Smart Schedule, EV Charging, Tariff compare, AI advisor, Goals & badges, Billing, Tenants, Multi-location, Download, Demo. Section titles (Anzeige, Web-Server, Preise & Tarif, TOU (Zeittarife), Alarm-Regeln, Batterie, Wetter, Prognose, KI-Berater, Ziele & Badges, Rechnung, Mieter) + group labels (Grundeinstellungen → Basic, Benachrichtigungen → Notifications, Integrationen → Integrations, Energie → Energy, Erweitert → Advanced) + every field label (Sprache, Nacht-Start, Tag/Nacht-Modus, Strompreis €/kWh, TOU-Tarif aktiv, Nur Werktags, Intervall (Std), Sigma-Schwelle, Load-Schwelle, Ein/Aus-Schwelle, Ladeleistung, Entladeleistung, Netzentgelt, Stromsteuer, Konzession, KWK, §19 StromNEV, Offshore, Anbieter-Marge, …) + every hint + every `opts:` select (Hell/Dunkel → Light/Dark, Tag/Nacht → Day/Night, Inkrementell → Incremental, Fix/Dynamisch → Fixed/Dynamic, Tarif/Anbieter → Tariff/Provider, …). Dropped two half-German hint strings in the spot_price and CO₂ sections that had leaked back in during earlier edits. User feedback: "ich hab wieder viel deutsch gefunden".
- **`services/webdash.py`**: `KI-Zusammenfassung` → `t('web.advisor.llm_summary','AI summary')`; `Dauer:` → `Duration:` in smart-schedule duration picker.
- **`services/tenant.py`**: `Stromverbrauch – {device}` description used in generated bills → `Electricity consumption – {device}`.
- **`services/entsoe.py`**: stray `{n} Bereich(e), Zone {zone}` log line → `{n} range(s), zone {zone}`.

### Notes
- **How to preview the new reports right now**: in Settings → Telegram / Email / Webhook, set the daily-summary "time" field to a minute ~2 minutes in the future, save, and wait. The first run will hit the daily report code path and you'll get the new text + 4-panel chart in Telegram, the styled HTML in your inbox (with the chart rendered inline), and the same JSON payload delivered to any webhook. For the monthly report, same flow — it fires on the 1st/2nd of the month past the configured time.
- **Email client support**: the HTML body is tested against Apple Mail, Gmail (web + iOS + Android), Outlook (Windows desktop, Outlook.com, Outlook mobile), Thunderbird and Proton Mail. The layout uses table-based columns + inline styles because Gmail and Outlook still strip `<style>` blocks in 2026.
- **Chart size**: the PNG is capped at 1100×700 px / ~150 KB so Telegram's photo endpoint accepts it without resampling and email attachments stay under the 25 MB limit of every major provider.
- **Nothing configured on disk has changed** — no new config fields, no migration needed. Existing installs get the enhanced reports automatically on the next scheduled fire.
...
- **Shelly Pro 3EM (Gen 2) sync 404.** `io/http.py` only knew the Gen2-style `/emdata/<id>/data.csv` URL, but some Pro 3EM firmware versions don't expose that REST path at all (browser hits 404 too). `download_csv()` now tries multiple variants in order: (1) `/emdata/<id>/data.csv?ts=…&end_ts=…&add_keys=true`, (2) the same without `add_keys`, (3) the Gen1 `/emeter/<id>/em_data.csv?ts=…&end_ts=…` URL, and (4) the `EMData.GetData` RPC method via `/rpc/`. The RPC response (whether `{keys, values}`, `{records:[…]}`, or `{data:[…]}`) is converted in-process into a CSV byte stream that `Database.insert_csv_bytes()` parses with the same code path as the REST download — short Pro 3EM key names (`a_total_act`, `b_total_act_ret`, …) are remapped to the canonical column names (`a_total_act_energy`, `b_total_act_ret_energy`, …) before write-out. Reported via Reddit /r/shellycloud (BornObsolete).
- **Shelly 3EM (Gen 1) "device offline" with password.** `services/sync.py` was unconditionally calling `EMData.GetRecords` (a Gen2+ RPC) before each chunk to align the start timestamp; on a Gen1 device this 404'd inside the sync loop and bubbled up as a connection error, then the chunk download itself used the Gen2 URL on the Gen1 device and failed again. Fixes: Gen1 devices now skip the `EMData.GetRecords` pre-flight, and `download_csv()` is called with `gen=int(device.gen)` so the URL is built correctly per generation (Gen1 → `/emeter/<id>/em_data.csv`, Gen2 → `/emdata/<id>/data.csv`). Combined with the credential cross-platform Digest+Basic auth from v16.15.0 this finally lets password-protected Gen1 3EMs sync.
- **`services/discovery.py` always marked Gen2 EM devices as `supports_emdata=False`.** That's the line that gates whether the sync loop even *tries* to fetch CSV history. The comment said "emdata CSV is mainly Gen1" — actually all Pro 3EM / Pro EM-50 devices support EMData history (and now we have an RPC fallback for the ones whose REST endpoint is missing). Newly probed Gen2 EM devices are now marked `supports_emdata=True`; switches/relays/lights stay False.
- **CRITICAL: every settings save reverted the session language back to German.** `web/app_context.py::reload_config()` ran `self.lang = normalize_lang(cfg.ui.language)` after loading the new config from disk. Disk still has whatever the user originally set (typically `de`), so any settings save — adding a device, changing an alert rule, editing pricing — silently flipped the in-memory language back to German, re-rendered the dashboard / plots / control HTML in German, and bounced every freshly-translated string back to its German form. **This was the actual root cause** of "in JEDEM Tab ist deutsch" reports: not missing translations (those were already in i18n.py), but the session language was being thrown away after every PUT. Fix: `reload_config()` no longer touches `self.lang` at all. Language is now managed exclusively by the new `set_language()` method, which the settings blueprint calls only when the user explicitly picks a new language in the UI. Boot still forces English; everything else preserves it.
- **More hard-coded German strings in webdash.py.** User reported "Kosten-Verteilung (Month)" still showing German on the Costs tab. Found and fixed: `Kosten-Verteilung` (cost breakdown), `Kosten-Ranking` (cost ranking, costs + standby panes), `Sigma-Verteilung` (sigma distribution, anomalies), `Aktiv` (standby active marker), `Ziel erreicht!` / `gespart` / `Ziel` (goals tab tiles), `Morgen ·` (forecast date prefix), `Letzte 6h vs. vorherige 6h` (CO₂ trend hint), `Schnitt` / `Max Abweichung` (anomaly metric labels), `6h-Prognose wird berechnet (Trend + Open-Meteo Wetter)` and the related forecast headers, `Anomalien gesamt`, `Schnitt W`, `Verlauf + Prognose`, `🔋 Batteriespeicher`, `Antwort ist kein JSON`, `HOCH` / `MITTEL` / `NIEDRIG` standby risk pills, four `t('web.costs.projected', 'Prognose')` and one `t('web.fc.forecast','Prognose')` fallbacks, `Dyn. Tarif`, `Festpreis` fallback in plot dynprice card, the spot-price footer notes (`Festpreis`, `Dein Tarif`, `Billigste Stunde`, `Teuerste Stunde`, `Durchschnitt`), and `' ct/kWh teurer' / 'Gleicher Preis wie Festpreis'` in the savings line.
- **Hard-coded German in `services/export.py` PDF reports.** 37 ternary lang-conditionals like `"Mittl. Leistung" if not is_en else "Avg Power"` were collapsed to English-only — this branch was hit by *every* non-English language including French, Polish, Czech etc., so e.g. a French user got "Mittl. Leistung" instead of either French or English. Same for: `"Tagesreport Energie"`, `"Monatsreport Energie"`, `"Stunde"` matplotlib axis label, the `or "Gesamt kWh"` / `or "Kosten"` t() fallbacks, and 35 more. PDFs now use English headers across the board (translated strings for the labels themselves still come through `t(lang, 'pdf.report.…')`).
- **`web/templates/settings.html`: 79 hard-coded German strings replaced with English.** 30 German T() fallbacks for keys that *do* exist in i18n.py (so the fallback was only ever shown when something went wrong) plus 47 raw `label:"…"` / `hint:"…"` / `opts:[…]` strings inside the settings field-schema arrays (which were *not* routed through `tField()` / `tHint()` because they're literals in the schema dict). Includes the `pv_surplus.consumers`, `tariff_schedule`, `billing.issuer`, `billing.customer`, `pricing.tariff_type`, `co2.entso_e_api_token`, `weather.lon`, `ev_charging.wallbox_device_key`, `demo.scenario` field labels and hints. Plus 9 toast / confirm dialogs in the device add/remove and firmware-update flows. So if for any reason a language map ever lacks one of these keys, users see English not German.

### Notes
- This is the release where the **language stickiness fix** lives. After installing v16.19.1, every previously-translated string in the dashboard, plots, control, settings and PDF export panes will *stay* in English (or whatever language the user explicitly picks via Settings → Display → Language) regardless of how many other settings they save afterwards. Restart the app once after the update to pick up the new `app_context.py` boot path.
- Pre-commit checks pass: `python -m py_compile` over the touched files, `scripts/check_templates_js.py` parses the 4 `<script>` blocks in the 2 templates cleanly.

## 16.19.0 - 2026-04-11
### Fixed
- **Every hard-coded German string in every Python file, translated.** User reported "in JEDEM Tab ist deutsch bei kosten angefangen bis zum letzten" — the dashboard tabs (Costs, CO₂, Forecast, Standby, Anomalies, Compare, Heatmap, EV Charging, Tariff, Goals, Tenants, Savings potential, …) still leaked German text even though the app is forced to English at boot. Root cause: dozens of `t('web.key','GERMAN FALLBACK')` calls in `services/webdash.py` used German fallbacks — and many of those keys are not registered in `i18n.py` at all, so the JS `t()` helper returned the German fallback verbatim. In addition, raw German HTML strings (not routed through `t()` at all) were embedded across tenant billing, spot-price blocks, EV log, standby hints, goals card, badges, advisor tips, ENTSO-E log lines, Telegram daily/monthly summaries, setup/sync/health blueprint error strings, and the zones.py country dropdowns.
- **`services/webdash.py`**: replaced 64 German fallbacks in `t()` calls (costs/month, standby/annual_cost, anom/by_device, fc/next_month, goals/streak_days, cmp/days, co2/green_score, hm/year_overview, loading, …) with proper English equivalents. Also scrubbed ~40 raw German HTML literals: tenant billing card (Mieter, Gemeinschaftsflächen, Einzug/Auszug, MwSt, Grundgebühr, Abrechnung berechnen, Fix/Dynamisch, "Keine Mieter angelegt"), the spot-price cheap-windows pane (Günstigste Zeitfenster, Waschmaschine/Wärmepumpe, Festpreis, ggü. Festpreis, Billigste/Teuerste Stunde, h Zeitblock, "Bei 2 kWh Verbrauch", "Preise inkl. Netzentgelt … MwSt. …"), EV log card (Ladeübersicht, Ladevorgänge, Datum/Zeit/Dauer, "Keine Ladevorgänge erkannt", Wallbox in Settings), savings-potential standby tip ("Standby-Stecker oder Zeitschaltuhr spart … €/Jahr"), Goals statistics (Tages ø, Tagesziel, Tage unter Ziel, Gespart), compare pane (Gesamt-Vergleich, Wochentags-Profil, Faktor relativ zum Durchschnitt), iOS Scriptable widget (Prüfe WiFi & IP-Adresse, Dashboard öffnen, Heute/Monat/Prognose/Leistung, Spotpreis/Festpreis, "Spotkosten heute", Energie, Menü), sync status panel (Status nicht verfügbar, Keine Geräte konfiguriert, nie, Letzte Sync-Zeiten, Status-Abruf fehlgeschlagen), timeout messages (Zeitüberschreitung), loading-msg / error-msg patterns (Lade…, Fehler:), per-tab 30-day consumption / week trend / last month / yearly projections, Dyn. Tarif, Festpreis, Öffnen, E-Auto Ladeprotokoll, Tarifvergleich, Batteriespeicher, KI-Energieberater, Ziele & Erfolge, Smart-Zeitplanung.
- **`services/ai_advisor.py`**: 13 German Tip titles + descriptions rewritten in English — Standby-Verbrauch → "Standby consumption", Günstigste Stunden nutzen → "Use the cheapest hours", Verbrauch gestiegen/gesunken → "Consumption rising/falling", Kältewarnung → "Cold warning", Hitzewarnung → "Heat warning", Dynamische Strompreise aktivieren → "Enable dynamic electricity prices", Solar-Monitoring einrichten → "Set up solar monitoring". Advisor LLM prompt also switched from "Du bist ein Energieberater … Antwort auf Deutsch" to "You are an energy advisor … Answer in English".
- **`services/gamification.py`**: all 10 `BADGE_DEFS` names + descriptions (Energiesparer, Sparchampion, Standby-Killer, Grüne Stunde, Solar-Champion, 7-Tage-Serie, 30-Tage-Serie, Nachtsparer, Spitzenvermeidung, Konstant sparsam) translated to English equivalents.
- **`web/blueprints/health.py`**: firmware update response strings ("Nur Beta-Update verfügbar", "Kein Firmware-Update verfügbar", "Update auf … gestartet", "Update läuft bereits") → English.
- **`web/blueprints/sync.py`**: CO₂ + spot-price manual-refresh JSON errors ("CO2 fetcher nicht aktiv", "CO2 Abruf gestartet", "Spot-Preis Fetcher nicht aktiv", "Spot-Preis Abruf gestartet") → English.
- **`web/blueprints/tenants.py`**: module docstring + "Keine Mieter konfiguriert" error → English.
- **`services/sync.py`**: 6 progress / log strings ("Gerät unterstützt kein EMData-CSV", "Bereich … Chunks", "Lade {a}-{b}", "Keine Daten (außerhalb Historie)", "Fehler:", "Chunk fehlgeschlagen", "fertig – Chunks OK", "unverändert", "Zeilen") → English.
- **`services/entsoe.py`**: CO₂ fetch service log lines ("CO₂ Bereinigung", "Fehler (Versuch)", "Chunk übersprungen", "CO₂ Import: Werte gespeichert", "CO₂ Import abgeschlossen mit N fehlgeschlagenen Chunks", "Lücken wurden mit Schätzwerten aufgefüllt", "Kraftwerksmix: Stunden nachgeladen", "Kraftwerksmix Backfill fehlgeschlagen") → English.
- **`web/background.py`**: Telegram daily + monthly summary builders — headers ("Tagesbericht", "Monatsbericht"), section titles ("Geräte", "Geräte-Ranking", "Gesamt", "Tagesschnitt", "Bester/Schlechtester Tag", "Monats-Hochrechnung", "Jahres-Hochrechnung", "Spitzenstunde", "Nacht-Grundlast", "24h-Verlauf", "Baumtage", "vs. Vortag", "vs. Vormonat", "Keine Daten vorhanden"), matplotlib chart titles & axis labels ("Stundenverlauf", "Stunde", "Geräte-Verbrauch", "Tagesverlauf", "Tag", "Geräte-Anteil", "Schnitt") → English.
- **`services/tariff_compare.py`**: hard-coded fallback tariff name "HT/NT Tarif" → "Day/Night TOU"; runtime-generated "Aktueller Tarif" / provider "Aktuell" → "Current tariff" / "Current".
- **`services/zones.py`**: all ~130 spot-price and CO₂ bidding-zone labels translated from German to English (Deutschland → Germany, Österreich → Austria, Großbritannien → Great Britain, Dänemark West → Denmark West, Italien – Mitte/Süd → Italy – Centre/South, Japan – Tokio → Japan – Tokyo, Südafrika → South Africa, Südkorea → South Korea, Vereinigte Arabische Emirate → United Arab Emirates, …) plus the optgroup labels ("Europa (Energy-Charts, ohne Anmeldung)" → "Europe (Energy-Charts, no key)", "USA (EIA, API-Key erforderlich)" → "USA (EIA, API key required)", "Australien (AEMO, ohne Anmeldung)" → "Australia (AEMO, no key)", "ENTSO-E (Europa, eigener API-Token)" → "ENTSO-E (Europe, own API token)").
- **`services/export.py`**: removed the `"Gerätedetails" if not is_en else "Device Details"` conditionals — now always uses the English title (DE/FR/PL/IT/ES/… users were getting German instead of their language anyway because the ternary only special-cased English).
- **`io/config.py`**: replaced the German billing placeholder defaults ("Firma Muster GmbH", "Musterstraße", "Kunde", "Kundenstraße") with neutral English ones ("Example Company Ltd", "Example Street", "Customer").

### Changed
- **`services/webdash.py` iOS widget (`_SCRIPTABLE_WIDGET_JS`)**: the raw Scriptable JS template that users import into Scriptable is now fully English — all card labels (Leistung, Heute, Monat, Prognose, Spotpreis, Festpreis, Energie, Spotkosten heute, Dashboard öffnen), the offline "Prüfe WiFi & IP-Adresse" fallback, and the header comments. Existing installs keep running — next `/widget.js` fetch delivers the new text automatically. User feedback: "das ändert die sprache nicht!! 📱 IOS WIDGET" — now it does.

### Notes
- **Process**: wrote a multi-pass Python audit script that greps `src/shelly_analyzer/**/*.py` for (a) raw umlauts `[äöüßÄÖÜ]` and (b) a dictionary of ~50 common German trigger words, excluding comments. After each round of fixes the scan is re-run until the only remaining hit is `i18n.py` itself (legitimate DE translation data). This is the same "search → fix → re-grep → repeat" loop I use for i18n hygiene.
- **Total**: ~240 German strings removed across 13 Python files (`webdash.py` alone: ~100 hits). `scripts/check_templates_js.py` passed post-edit: 2 templates, 4 `<script>` blocks, all parse cleanly.
- **No new i18n keys were added this release** — the existing `t()` infrastructure already covers every touch-point; the bug was that German strings had slipped in at write-time. Going forward, rely on the pre-commit umlaut scan (see `feedback_i18n_always.md`).

## 16.18.0 - 2026-04-11
### Fixed
- **105 more hard-coded German strings in `/settings` translated.** Deep audit + systematic fix of every untranslated label / hint / option / button / toast / confirm across the settings page. User reported the Health Check, Spot Price, Web Server, Pricing & Tariff sections (and basically every settings menu) still showed German in non-DE languages — this release wires them all through the i18n system. Breakdown:
  - **`renderField` now auto-translates via 3 new helpers**: `tSubField(parentKey, sf)` for nested list-field labels (tou.rates.name, pricing.tariff_schedule.start_date, pv_surplus.consumers.device_key, tenant.tenants.name, etc. — 17 sub-field labels), `tHint(f)` for the hint text under each field (17 hints: `settings.hint.ui.live_web_token`, `settings.hint.spot_price.tariff_type`, `settings.hint.co2.entso_e_api_token`, `settings.hint.billing.customer.address`, …), and `tOpt(fieldKey, optKey, optLabel)` for every `<option>` label in flat selects (32 options across `ui.theme`, `ui.plot_theme_mode`, `ui.live_daynight_mode`, `ui.autosync_mode`, `ui.telegram_detail_level`, `spot_price.tariff_type`, `spot_price.primary_api`, `advisor.llm_provider`, `demo.scenario`, `tariff_type`). Lookup key template: `settings.field.<parent>.<sub>`, `settings.hint.<field>`, `settings.opts.<field>.<value>`.
  - **Custom `renderHealth()`**: `<h2>Health Check</h2>`, "Ping aller Shellys + Firmware-Status", "🩺 Jetzt prüfen", "⬆ Alle updaten", "Klick „Jetzt prüfen" für aktuellen Status" — all 6 strings now `T(settings.health.*, …)`.
  - **Custom `renderAlerts()`**: section title, loading state, "Neue Regel" / "Aktualisieren" buttons, inside the `<template>` every field label ("Name / ID", "Aktiv", "Gerät", "Metrik", "Operator", "Schwelle", "Dauer (Sek)", "Cooldown (Sek)", "Popup/Beep", "Nachricht", "E-Mail"), "Keine Regeln konfiguriert" empty state, the "Hohe Last" default message for new rules, `confirm("Regel … löschen?")`, "✓ Gespeichert" toast — 18 strings.
  - **Custom `renderAbout()`**: `<h2>Über</h2>`, "App-Version", "Config-Version", "Lade…", "Lade DB-Statistik…" — 5 strings.
  - **Device-card inline labels in `renderDevices()`**: "Name", "Host / IP", "Schlüssel (read-only)", "EM-ID", "Typ" + 3 kind options (Energy Meter / Switch / Relais / Unbekannt), "Generation" + "Auto" option, "Phasen" + 3 phase options (1-phasig / 2-phasig / 3-phasig), "Modell" + placeholder "z. B. SHEM-3", "HTTP user (password-protected Shellys)", "HTTP password" + 2 placeholders ("(saved – type to change)", "leave empty for unprotected device"), "EMData / historical data supported", "Status", "Online", "Offline / no data", "password set" — 21 strings all under `settings.devcard.*`.
  - **`triggerFirmwareUpdate()` / `updateAllWithPendingFirmware()` / `deleteAlertRow()`**: the 4 German `confirm()` dialogs + 4 toasts now go through T() with named params (`{name}`, `{n}`, `{id}`). Previously the user saw `Firmware-Update für "Shelly" starten?` even in French.
  - **`<option value="*">Alle Geräte</option>`** in the alert rule device picker → `T("settings.alerts.all_devices", …)`.
  - **`(keine)` fallback** in `select-device` dropdowns → `T("settings.opts.device.none", …)`.
  - **Inline "Fehler" strings** in 4 error-display branches of `renderUpdates()` / `renderSSL()` / `checkUpdateStatus()` → `T("settings.err", "Fehler")`.
  - **105 new i18n keys registered in `i18n.py`** (59 from `T()` calls, 17 hints, 32 select options, minus duplicates). All with proper German AND proper English values — I also fixed 11 EN placeholders that had inherited German during the first pass (Fix→Fixed, Monat→Month, Wärmepumpe→Heat pump, aWATTar (nur DE/AT)→aWATTar (DE/AT only), Ollama (lokal)→Ollama (local), Haushalt→Household, PV-Haus→PV home, Mieter→Tenant metering, Ausführlich→Detailed, Kurz→Brief, Energy-Charts (alle EU)→Energy-Charts (all EU)).
  - **DE + EN maps both at 1975 keys**, perfectly synced. The 7 partial-language overlays (fr/pt/it/pl/cs/ru) fall back to English for every new key, so Français / Italiano / Polski / Česky / Русский / Português users now see English translations of the previously-German strings instead of leaking German through the `de → en → requested` chain.
  - **`scripts/check_templates_js.py` still passes** — all 4 `<script>` blocks in the 2 HTML templates parse cleanly after the edit.
...
- **Global spot market prices.** Three new providers feed the existing `spot_prices` table:
  - **EIA (US)** via `api.eia.gov/v2/electricity/wholesale/daily-region-data` — covers 13 wholesale regions (`US-CAL`, `US-CAR`, `US-CENT`, `US-FLA`, `US-MIDA`, `US-MIDW`, `US-NE`, `US-NW`, `US-NY`, `US-SE`, `US-SW`, `US-TEN`, `US-TEX`). Requires a free API key registered at eia.gov/opendata/register.php; new `spot_price.eia_api_key` config field.
  - **AEMO (AU)** via `aemo.com.au/aemo/api/v1/visdata/nem/spot` — covers the five NEM regions (`AU-NSW`, `AU-QLD`, `AU-SA`, `AU-TAS`, `AU-VIC`). No API key required, 30-minute rolling window.
  - Prices are converted from USD / AUD to EUR/MWh via daily ECB rates from `api.frankfurter.app` (cached per-day, process-local) so the stored values are directly comparable with EU spot prices and the existing cost math works unchanged.
  - `SpotPriceFetchService._fetch_chunk()` auto-dispatches based on the zone prefix — `US-*` → EIA, `AU-*` → AEMO, everything else → Energy-Charts / aWATTar. The user can't misconfigure themselves into an empty-fetch loop by picking the wrong "API" dropdown.
- **Global CO₂ intensity.** New `services/electricity_maps.py` wraps the Electricity Maps v3 API (`api.electricitymap.org`) and writes into the same `co2_intensity` table as the existing ENTSO-E integration:
  - **92 global zones** grouped by continent: North America, South America, Asia, Oceania, Africa, Middle East, plus all EU zones. Covers the USA (CAISO, ERCOT, ISO-NE, NYISO, PJM, MISO, BPA, SRP, Duke, FPL), Canada (ON, QC, BC, AB), Mexico, Brazil, Argentina, Chile, Colombia, Peru, Japan (7 subregions), South Korea, Taiwan, China, India (4 subregions), Indonesia, Malaysia, Philippines, Thailand, Vietnam, Australia (6 states), New Zealand, South Africa, Egypt, Morocco, Nigeria, Kenya, Israel, Turkey, UAE, Saudi Arabia, and Singapore/Hong Kong.
  - Requires a free API key from electricitymap.org; new `co2.electricity_maps_api_key` config field.
  - `BackgroundServiceManager._start_co2_fetcher()` now dispatches to the `ElectricityMapsFetchService` when the selected CO₂ zone uses the hyphen format (e.g. `US-CAL-CISO`, `JP-TK`) and an Electricity Maps key is present. Underscore zones (`DE_LU`, `SE_4`) keep using ENTSO-E.
- **Grouped zone dropdowns in settings.** The spot-price and CO₂ zone selectors now render `<optgroup>` elements sourced from `/api/zones` (`spot_grouped`, `co2_grouped`). The spot selector has three groups (Europa · USA · Australien); the CO₂ selector has two (ENTSO-E · Electricity Maps). New API-key password fields for EIA and Electricity Maps sit right next to their respective zone pickers, with hints linking to the free-registration pages. All three new secrets (`co2.entso_e_api_token`, `co2.electricity_maps_api_key`, `spot_price.eia_api_key`) are masked as `***` in `GET /api/settings` and restored from the original config on `PUT` so they're never exposed to the browser after the first save.
...
- **Widget auto-domain** – `widget_domain` auto-detected from SSL certificate CN (e.g. `energie.example.com`). Baked into the Scriptable script download so no manual parameter needed.
