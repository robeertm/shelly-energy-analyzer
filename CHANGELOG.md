...
## 16.42.0 - 2026-07-24
### Added
- **Zähler-Kaskade (Meter hierarchy).** Die Kalibrierung war bisher rein pro Gerät. Wer mehrere Unterzähler an EINEM Hauptzähler hat (z. B. „Haus" + „Wallbox"), verglich den Hauptzähler-Stand gegen ein EINZELNES Untergerät — das ergibt einen unsinnigen Faktor (der Hauptzähler misst ja beide zusammen). Neu bildet man eine beliebige Zähler-Struktur ab: beliebig viele **Hauptzähler** (mit Name + Seriennummer), jedes Shelly-Gerät wird seinem übergeordneten Zähler zugeordnet (Hauptzähler oder ein anderes Messgerät → beliebige Kaskadentiefe). Ein Hauptzähler kalibriert immer gegen die **Summe seiner direkten `em`-Kinder** — Schalter und tiefer liegende Unter-Unter-Zähler zählen nicht mit (keine Doppelzählung).
  - Backend: neues `MainMeter` (id/name/serial) + `AppConfig.main_meters`, `DeviceConfig.parent`; Endpunkte `GET/POST /api/meters`, `DELETE /api/meters/<id>` (löst zugeordnete Geräte statt sie zu verwaisen). Vollständig **rückwärtskompatibel**: bestehende (flache) Konfigurationen laden unverändert (`parent=""`, `main_meters=[]`).
- **Zählerstand-Logbuch statt Start/Ende-Eingabe.** Man loggt je Hauptzähler von Zeit zu Zeit den abgelesenen **Zählerstand** (`MeterReading{ts,kwh}`); die App leitet den Kalibrierfaktor automatisch aus der Abweichung ZWISCHEN aufeinanderfolgenden Ablesungen ab und wendet ihn **datiert und rückwirkend** genau auf diesen Zeitraum an. Eine einzelne Ablesung = Startwert (noch kein Faktor). Ältere Zählerstände lassen sich jederzeit nachtragen (der betroffene Zeitraum wird neu berechnet). **Trend-Chart** der Abweichung über die Zeit, Ablesungs-Tabelle mit „Faktor ab hier" + Löschen. Endpunkte `POST/DELETE /api/meters/<id>/reading`. Driftfrei durch rohe (unkompensierte) kWh in der Rückrechnung (`query_hourly(compensate=False)`).
- **Faktor für Verbrauch VOR der ersten Ablesung.** Diese Vorzeit hat keine eigene Messung und wird mit dem **verbrauchsgewichteten Gesamtfaktor** (Gesamt-Zähler-Δ / Gesamt-roh-Δ über alle Intervalle) extrapoliert — nicht dem einfachen Prozent-Mittel, das kurze/verrauschte Intervalle übergewichtet. Als datierte Vorzeit-Stufe umgesetzt, damit die bestehende Step-Funktion sie rückwirkend anwendet.
- **Setup-Wizard: Schritt „Geeichter Hauptzähler".** Direkt nach dem Geräte-Schritt kann man den offiziellen (geeichten) Netzzähler mit **Seriennummer** anlegen, die zugehörigen Shelly-Unterzähler zuordnen und den **aktuellen Zählerstand als Startwert** eintragen — so ist ab Tag 1 eine geeichte Basis vorhanden. Überspringbar; nur im Erst-Setup.
- Neue UI-Texte (`cal.*`, `setup.mainmeter.*`) in DE + EN; voller 9-Sprachen-Sweep als Follow-up.
...
### Docs
- **README:** Neuer Abschnitt „Maintenance via CI" — beschreibt, dass Änderungen per GitHub-Issue beauftragt werden und der Deploy automatisch auf Roberts VM läuft, ausdrücklich ohne ein neues Release.
